    ...
    case kTfLiteInt8: {
      switch (filter->type) {
        ...
        case kTfLiteInt8: {
          ...
          if(output_depth * output_height * output_width < TEMP_BUFFER_SIZE) {
			  ...
			  depthconv2d_8bitconv_16bitquant(
          ...
          if(stride_width == 1){
        	  depthconv2d_noskip_asm_non_interleaved_padding1_8bitconv_16bitquant(
        		  tflite::micro::GetTensorData<int8_t>(input),			//Input buffer
				  tflite::micro::GetTensorData<int8_t>(output),			//Output buffer
				  tflite::micro::GetTensorData<int8_t>(filter),			//Weights buffer
				  tflite::micro::GetOptionalTensorData<int32_t>(bias),	//Bias
        	  ...
        	  if(filter_height==10){
				  depthconv2d_skip_asm_non_interleaved_8x10(
					  ...
					  depthconv2d_skip_asm_non_interleaved_padding1_8bitconv_16bitquant(
						  tflite::micro::GetTensorData<int8_t>(input),			//Input buffer
						  tflite::micro::GetTensorData<int8_t>(output),			//Output buffer
						  tflite::micro::GetTensorData<int8_t>(filter),			//Weights buffer
						  tflite::micro::GetOptionalTensorData<int32_t>(bias),	//Bias
		  ...
		  printf("\tNumber of cycles to run Depthwise Layer : \t%ld \n", cyc);
#endif
