...
void TestAveragePoolQuantized(
  ...
  //const TFLMRegistration registration = Register_AVERAGE_POOL_2D(); //original line
  const TfLiteRegistration_V1 registration = Register_AVERAGE_POOL_2D();
  ValidatePoolingGoldens(tensors, tensors_size, registration, filter_height,
                         filter_width, stride_height, stride_width,
                         expected_output_data, output_dims_count, padding,
...
void TestMaxPoolFloat(int* input_dims_data, const float* input_data,
  ...
  //const TFLMRegistration registration = Register_MAX_POOL_2D(); //original line
...
TF_LITE_MICRO_TEST(Stride4X4SimpleMaxPoolInt16) {
  int input_shape[] = {4, 1, 4, 4, 1}; //input_shape[]={dims_size,batch,input_height,input_width,depth}
  const int16_t input_values1[] = {0, 6, 2, 4, 3, 2, 10, 7,12,9,5,8,11,15,17,1};
  const int filter_width = 4;
  const int filter_height = 4;
  const int stride_width = 4;
  const int stride_height = 4;
  const int16_t golden1[] = {17};
