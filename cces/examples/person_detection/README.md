...
* Expected Input: Requires `is_person.bin` in `src\input`. To run it on a custom image, first convert the image to binary using the script in `cces\Utils\data\scripts`. Please follow the Readme in `cces\Utils\data\person_detection` to generate the input.
