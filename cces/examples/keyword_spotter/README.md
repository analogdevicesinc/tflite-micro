...
* Expected input: Requires keyword_audio.bin in `kws_fileio/src/input` folder. Follow the Readme in `cces\Utils\data\keyword_spotter` to generate the input file for testing the application.
...
* The sample wav files from `cces\Utils\data\keyword_spotter\sample_data` can be used for testing the realtime application.
...
To quickstart the kws example, this project allows headless building and flashing of the application.
The commands are bash commands and need to be run in such commandline tools. Tested the commands using git bash.
To build, run the following from `kws_fileio` or `kws_realtime` directory:
```
make
```
To flash the realtime application, the following command can be run from `kws_realtime` directory.
```
make flash
```
