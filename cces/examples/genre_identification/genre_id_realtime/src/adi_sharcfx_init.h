...
/*==============  D E F I N E S  ===============*/
#define GAIN_FACTOR 				2.5f
#define CIRCULAR_BUFFER_LENGTH      NUM_HOPS * WINDOWS_PER_FRAME
#define DECIMATION_FACTOR 			1
#define AUDIO_COUNT                 ((int)(DECIMATION_FACTOR * 2 * HOP_SIZE)) /* 1 * 2(mono/stereo) * HOP_SIZE */
#define SCALE_FACTOR 				16777216 	/*this is the scaling factor to quantize actual real world analog samples to digital since it is a 24 bit dac, this scaling factor corresponds to 2^24 */
...
extern int g_audio_data_input[AUDIO_COUNT*CIRCULAR_BUFFER_LENGTH];//this holds the raw input data
extern int g_audio_data_output[AUDIO_COUNT*CIRCULAR_BUFFER_LENGTH];//holds raw audio output data
extern volatile uint32_t pReadPtr;
extern volatile uint32_t pWritePtr;
extern uint32_t pProcessReadPtr;
extern uint32_t pProcessWritePtr;
extern float g_audio_windowed_data_input[FFT_SIZE];
extern int max_index;
extern int int_SP0ABufferZero[AUDIO_COUNT];
