...
static int   int_SP0ABuffer1[AUDIO_COUNT]__attribute__((section(".L1.data"), aligned(32)));//*2 for stereo, *2 for 48KHz->24KHz
//__attribute__((section(".L1.data")))
static int   int_SP0ABuffer2[AUDIO_COUNT]__attribute__((section(".L1.data"), aligned(32)));

//__attribute__((section(".L1.data")))
static int   int_SP0ABuffer4[AUDIO_COUNT]__attribute__((section(".L1.data"), aligned(32)));
//__attribute__((section(".L1.data")))
static int   int_SP0ABuffer5[AUDIO_COUNT]__attribute__((section(".L1.data"), aligned(32)));

/* Destination Sport PDMA list */
__attribute__((section(".L1.data"),aligned(32)))
...
static uint8_t SPORTMemory4A[ADI_SPORT_MEMORY_SIZE]__attribute__((section(".L1.data"),aligned(32)));
static uint8_t SPORTMemory4B[ADI_SPORT_MEMORY_SIZE]__attribute__((section(".L1.data"),aligned(32)));
...
bool isBufferFull() {
    return ((pReadPtr + 1) % (NUM_HOPS * WINDOWS_PER_FRAME)) == (pProcessReadPtr % (NUM_HOPS * WINDOWS_PER_FRAME));
}
...
)
{
    ...
    switch (nEvent)
    {
        ...
        case ADI_SPORT_EVENT_RX_BUFFER_PROCESSED:
			...
			{
				if (!isBufferFull()){
					int nReadLocation = (pReadPtr) % (CIRCULAR_BUFFER_LENGTH);//circular buffer loopback to beginning
					// Ensure we have enough space to read AUDIO_COUNT samples from g_audio_data_input
					// and that AUDIO_COUNT samples can safely fit into int_SP0ABuffer4
					if (((nReadLocation * AUDIO_COUNT + AUDIO_COUNT) <= (sizeof(g_audio_data_input) / sizeof(g_audio_data_input[0])))  && (AUDIO_COUNT <= (sizeof(int_SP0ABuffer4)/sizeof(int_SP0ABuffer4[0])))) {
						...
						for(int i = 0; i<AUDIO_COUNT; i++){
							g_audio_data_input[(nReadLocation * AUDIO_COUNT)+i] = (int)int_SP0ABuffer4[i];
						}
			...
			else if(RxCallbackCount==2)
			{
				if (!isBufferFull()){
					...
					// Ensure we have enough space to read AUDIO_COUNT samples from g_audio_data_input
					// and that AUDIO_COUNT samples can safely fit into int_SP0ABuffer4
					if (((nReadLocation * AUDIO_COUNT + AUDIO_COUNT) <= (sizeof(g_audio_data_input) / sizeof(g_audio_data_input[0])))  && (AUDIO_COUNT <= (sizeof(int_SP0ABuffer5)/sizeof(int_SP0ABuffer5[0])))) {
						/* pragma no_simd is added because we noticed when memcpy or loop vectorization is enabled then there is memory corruption
						   and the inference results get corrupted */
						#pragma no_simd
						for(int i = 0; i<AUDIO_COUNT; i++){
							g_audio_data_input[(nReadLocation * AUDIO_COUNT)+i] = (int)int_SP0ABuffer5[i];
						}
						pReadPtr++;//updated read buffer location
					}
					else {
        ...
        case ADI_SPORT_EVENT_TX_BUFFER_PROCESSED:
			...
			if(TxCallbackCount==1)
			{
				...
				if(pWritePtr < pReadPtr) {
					int nWriteLocation = (pWritePtr) % (CIRCULAR_BUFFER_LENGTH);//circular buffer loopback to beginning
					/* pragma no_simd is added because we noticed when memcpy or loop vectorization is enabled then there is memory corruption
					   and the inference results get corrupted */
					#pragma no_simd
					for(int i = 0; i<AUDIO_COUNT; i++){
...
int Sport_Init(void)
{
    ...
    memset(int_SP0ABuffer1, 0, sizeof(int_SP0ABuffer1));
	memset(int_SP0ABuffer2, 0, sizeof(int_SP0ABuffer2));
	memset(int_SP0ABuffer4, 0, sizeof(int_SP0ABuffer4));
	memset(int_SP0ABuffer5, 0, sizeof(int_SP0ABuffer5));
