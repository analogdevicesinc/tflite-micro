...
* Follow the steps mentioned in the README file in `cces\Utils\automated-model-conversion\dtln` to generate the models for the application. The batch script will download, convert the float32 models and place them in the relevant locations for the project to access it. 
...
* Expected input: Requires noisy_audio.bin in `denoiser_fileio/src/input` folder. Follow the Readme in `cces\Utils\data\denoiser` to generate the input file for testing the application.
* Expected output: denoised audio signal will be stored in the `denoiser_fileio/src/output` folder under `denoised_audio.bin`. 
* The output bin file `denoised_audio.bin` can be converted back to wav format using the `convert_bin_to_wav.py` script from the Utils folder (`cces\Utils\data\scripts`). Please follow the Readme in `cces\Utils\data\denoiser`. 
...
* Expected Input: Follow the Readme in `cces\Utils\data\denoiser` to download and use the input file. You can use any custom noisy file of your choise for testing.

### Headless build and flash
To quickstart the denoiser example, this project allows headless building and flashing of the application.
The commands are bash commands and need to be run in such commandline tools. Tested the commands using git bash.
To build, run the following from `denoiser_fileio` or `denoiser_realtime` directory:
```
make
```
To flash the realtime application, the following command can be run from `denoiser_realtime` directory.
```
make flash
```
