...
2. **Defining the initialization file**
    ...
    `cces\Utils\flashing-tools\init_code_sharc_fx`
    ...
    Ensure the entire path to the `init_code_sharc_fx.dxe` file (generated after running the `init_code_sharc_fx CCES` application) is added. `init_code_sharc_fx.dxe` gets generated in Release or Debug folder after building `cces\Utils\flashing-tools\init_code_sharc_fx` CCES porject.
...
1. **Bring in the loader file**
	Copy the built loader file(`.ldr`) to `cces\prebuilts`.
