...
class Set():
    ...
    def make_dataset(self):
        ...
        for i, genre in enumerate(self.data.GENRES):
            ...
            print(f"{genre_df}, {len(genre_df)}") #2100
            n_train = int(0.7 * len(genre_df))
            n_test = int(0.1 * len(genre_df))
            n_valid = len(genre_df) - n_train - n_test
            train_records.append(genre_df.iloc[:n_train].values)
            valid_records.append(genre_df.iloc[n_train:n_train + n_valid].values)
            test_records.append(genre_df.iloc[n_train + n_valid:].values)
