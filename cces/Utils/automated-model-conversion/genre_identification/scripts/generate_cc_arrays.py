...
#   - Modified array naming convention to use 'g_' prefix.
# ==============================================================================
...
def generate_file(out_fname, array_name, array_type, array_contents, size):
  ...
  if out_fname.endswith('.cc'):
    ...
    #     out_fname.split('genfiles/')[-1].replace('.cc', '.h')).replace('TFLM_models\\', ''))
    header_path = os.path.splitext(os.path.basename(out_fname))[0] + '.h'
    out_cc_file.write(f'#include "{header_path}"\n\n')
    ...
    out_hdr_file.write('extern const {} {}[]__attribute__((section(".L3.data"), aligned(16)));\n'.format(
        array_type, array_name))
    out_hdr_file.close()
  else:
...
def get_array_name(input_fname):
  ...
  base_array_name = 'g_'+ input_fname.split('_')[0]#+ abs_fname.split('.')[-2].split('/')[-1]
  if input_fname.endswith('.tflite'):
