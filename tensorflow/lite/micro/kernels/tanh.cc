...
TfLiteStatus CalculateArithmeticOpData(TfLiteContext* context, TfLiteNode* node,
  ...
  if (input->type == kTfLiteInt16) {
    ...
    if (param_scale_pot) {
      ...
      double multiplier =
          static_cast<double>(input->params.scale) * 4096.0;/*Scaling by 3 not required as LUT is not being used in optimized case*/
#else
      double multiplier =
  ...
  switch (input->type) {
    ...
    case kTfLiteInt16: {
       ...
       reference_integer_ops::Tanh(
          ...
          PRINT_INFO("\tNumber of cycles to run Tanh(INT_16) : \t%ld \n", cyc);
#endif
      return kTfLiteOk;
    } break;
    case kTfLiteInt8: {
