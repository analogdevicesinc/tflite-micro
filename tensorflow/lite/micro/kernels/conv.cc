...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (input->type) {  // Already know in/out types are same.
    ...
    case kTfLiteInt8: {
...
#endif
#if defined(USE_OPTIMIZED_1x1_CONV) || defined(USE_OPTIMIZED_3x3_CONV)
      ...
      if(dilation_width_factor==1 && dilation_height_factor==1 && BufSizeNeeded<=TEMP_BUFFER_SIZE_L1)
      {
#ifdef USE_REORDERED_WEIGHTS_SCHEME
          if (filter_height == 1 && filter_width == 1)
          {
              adi_sharcfx_conv2d_kernel1x1_int8_reordered_weights(
