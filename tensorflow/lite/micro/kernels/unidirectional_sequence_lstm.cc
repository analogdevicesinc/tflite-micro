  ...
  switch (activation_type) {
    case kTfLiteFloat32: {
      ...
      EvalLstm<float, float, float, float>(op_data, kernel_content, buffers);
          ...
          PRINT_INFO("\tNumber of cycles to run uni_dir_seq_lstm float Layer: \t %lu \n", cyc);
#endif
      break;
    }
    case kTfLiteInt8: {
...
#ifdef DISPLAY_CYCLE_COUNTS
          STOP_CYCLE_COUNT (cyc, var);
          PRINT_INFO("\tNumber of cycles to run uni_dir_seq_lstm [8(activation)x8(weight)->16(cell) LSTM with 32 bits bias] Layer: \t %lu \n", cyc);
#endif
          break;
        }
        default: {
...
#ifdef DISPLAY_CYCLE_COUNTS
          STOP_CYCLE_COUNT (cyc, var);
          PRINT_INFO("\tNumber of cycles to run uni_dir_seq_lstm [16(activation)x8(weight)->16(cell) LSTM with 64 bits bias] Layer: \t %lu \n", cyc);
#endif
