...
inline int Non_Interleaved_Offset(const RuntimeShape& shape, int i0, int i1, int i2, int i3) {
  TFLITE_DCHECK_EQ(shape.DimensionsCount(), 4);
  const int* dims_data = reinterpret_cast<const int*>(shape.DimsData());
//  TFLITE_DCHECK((dims_data[0] == 0 && i0 == 0) ||
//                (i0 >= 0 && i0 < dims_data[0]));
//  TFLITE_DCHECK((dims_data[3] == 0 && i1 == 0) ||
//                (i1 >= 0 && i1 < dims_data[3]));
//  TFLITE_DCHECK((dims_data[2] == 0 && i2 == 0) ||
//                (i2 >= 0 && i2 < dims_data[2]));
//  TFLITE_DCHECK((dims_data[1] == 0 && i3 == 0) ||
//                (i3 >= 0 && i3 < dims_data[1]));
  return ((i0 * dims_data[3] + i1) * dims_data[2] + i2) * dims_data[1] + i3;
}
        ...
        for (int channel = 0; channel < depth; ++channel) {
          ...
          for (int filter_y = filter_y_start; filter_y < filter_y_end;
            ...
            for (int filter_x = filter_x_start; filter_x < filter_x_end;
              ...
              acc +=
#ifdef USE_OPTIMIZED_AVGPOOL
                  input_data[Non_Interleaved_Offset(input_shape, batch, channel, in_y, in_x)];
#else
