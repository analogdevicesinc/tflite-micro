...
TF_LITE_MICRO_TEST(Int8Input1x16x16x32Filter12x3x3x32PaddingShouldMatchGolden) {
  using tflite::Int8ConvCommonInput;
  using tflite::Int8ConvCommonFilter;
  using tflite::kConvInput1x16x16x32Golden1x16x16x12;
  using tflite::kConvCommonZeroBias;
  using tflite::input_quantized1x16x16x32;
  using tflite::filter_quantized12x1x16x16x32;
  using tflite::output_quantized1x16x16x12;
  using tflite::golden_quantized1x16x16x12;
  using tflite::bias_quantized16x16x32Inp;
  constexpr int kNumFilters = 12;
  int input_shape[] = {4, 1, 16, 16, 32};  // [len,N,H,W,C]
  int filter_shape[] = {4, kNumFilters, 3, 3, 32};
  ...
  conv_params.stride_height = 1;
  conv_params.stride_width = 1;
  conv_params.padding = kTfLitePaddingSame;//kTfLitePaddingValid;
  ...
  // Create per-tensor quantized int8_t output tensor.
  TfLiteTensor output_tensor = tflite::testing::CreateQuantizedTensor(
		  output_quantized1x16x16x12, output_dims, output_scale, output_zero_point);
  // Set zero point and scale arrays with a single element for each.
  int output_zero_points[] = {1, output_zero_point};
  float output_scales[] = {1, output_scale};
  TfLiteAffineQuantization output_quant = {
  ...
  tflite::Quantize(kConvInput1x16x16x32Golden1x16x16x12, golden_quantized1x16x16x12, output_dims_count,
                   output_scale,
				   output_zero_point);
  ...
  TF_LITE_MICRO_EXPECT_EQ(
      kTfLiteOk, tflite::testing::ValidateConvGoldens(
                     tensors, kTensorsSize, golden_quantized1x16x16x12, output_dims_count,
                     &conv_params, tflite::Register_CONV_2D(), output_quantized1x16x16x12,
                     kQuantizationTolerance));


}

TF_LITE_MICRO_TEST(Int8Input1x15x15x35Filter12x3x3x35PaddingShouldMatchGolden) {
  using tflite::Int8ConvCommonInput;
  using tflite::Int8ConvCommonFilter;
  using tflite::kConvInput1x15x15x35Golden1x15x15x12;
  using tflite::kConvCommonZeroBias;
  using tflite::input_quantized1x15x15x35;
  using tflite::filter_quantized12x1x15x15x35;
  using tflite::output_quantized1x15x15x12;
  using tflite::golden_quantized1x15x15x12;
  using tflite::bias_quantized15x15x35Inp;
  constexpr int kNumFilters = 12;
  int input_shape[] = {4, 1, 15, 15, 35};  // [len,N,H,W,C]
  int filter_shape[] = {4, kNumFilters, 3, 3, 35};
  int bias_shape[] = {1, kNumFilters};
  int output_shape[] = {4, 1, 15, 15, kNumFilters};
  ...
  // Create per-tensor quantized int8_t input tensor.
  TfLiteTensor input_tensor = tflite::testing::CreateQuantizedTensor(
		  Int8ConvCommonInput, input_quantized1x15x15x35, input_dims, input_scale, input_zero_point);
  // Set zero point and scale arrays with a single element for each.
  int input_zero_points[] = {1, input_zero_point};
  float input_scales[] = {1, input_scale};
  TfLiteAffineQuantization input_quant = {
  ...
  // Create per-tensor quantized int8_t filter tensor.
  TfLiteTensor filter_tensor = tflite::testing::CreateQuantizedTensor(
		  Int8ConvCommonFilter, filter_quantized12x1x15x15x35, filter_dims, filter_scale,
      filter_zero_point);
  // Set zero point and scale arrays with a single element for each.
  int filter_zero_points[] = {1, filter_zero_point};
  float filter_scales[] = {1, filter_scale};
  ...
  // Create per-tensor quantized int32_t bias tensor.
  tflite::SymmetricQuantize(kConvCommonZeroBias, bias_quantized15x15x35Inp, kNumFilters,
                            input_scale * output_scale);
  TfLiteTensor bias_tensor =
      tflite::testing::CreateTensor(bias_quantized15x15x35Inp, bias_dims);
  ...
  // Create per-tensor quantized int8_t output tensor.
  TfLiteTensor output_tensor = tflite::testing::CreateQuantizedTensor(
		  output_quantized1x15x15x12, output_dims, output_scale, output_zero_point);
  // Set zero point and scale arrays with a single element for each.
  int output_zero_points[] = {1, output_zero_point};
  float output_scales[] = {1, output_scale};
  TfLiteAffineQuantization output_quant = {
  ...
  tflite::Quantize(kConvInput1x15x15x35Golden1x15x15x12, golden_quantized1x15x15x12, output_dims_count,
                   output_scale,
				   output_zero_point);
  ...
  TF_LITE_MICRO_EXPECT_EQ(
      kTfLiteOk, tflite::testing::ValidateConvGoldens(
                     tensors, kTensorsSize, golden_quantized1x15x15x12, output_dims_count,
                     &conv_params, tflite::Register_CONV_2D(), output_quantized1x15x15x12,
                     kQuantizationTolerance));


}
//
TF_LITE_MICRO_TEST(Int8Input1x16x16x28Filter12x3x3x28PaddingSame) {
    using tflite::Int8ConvCommonInput;
    using tflite::Int8ConvCommonFilter;
    using tflite::kConvInput1x16x16x28Golden1x16x16x12;
    using tflite::kConvCommonZeroBias;
    using tflite::input_quantized1x16x16x28;
    using tflite::filter_quantized12x1x16x16x28;
    using tflite::output_quantized1x16x16x12;
    using tflite::golden_quantized1x16x16x12;
    using tflite::bias_quantized16x16x28Inp;
    constexpr int kNumFilters = 12;
    int input_shape[] = {4, 1, 16, 16, 28};  // [len,N,H,W,C]
    int filter_shape[] = {4, kNumFilters, 3, 3, 28};
    int bias_shape[] = {1, kNumFilters};
    int output_shape[] = {4, 1, 16, 16, kNumFilters};
    ...
    // Create per-tensor quantized int8_t input tensor.
    TfLiteTensor input_tensor = tflite::testing::CreateQuantizedTensor(
  		  Int8ConvCommonInput, input_quantized1x16x16x28, input_dims, input_scale, input_zero_point);
    // Set zero point and scale arrays with a single element for each.
    int input_zero_points[] = {1, input_zero_point};
    float input_scales[] = {1, input_scale};
    TfLiteAffineQuantization input_quant = {
    ...
    // Create per-tensor quantized int8_t filter tensor.
    TfLiteTensor filter_tensor = tflite::testing::CreateQuantizedTensor(
  		  Int8ConvCommonFilter, filter_quantized12x1x16x16x28, filter_dims, filter_scale,
        filter_zero_point);
    // Set zero point and scale arrays with a single element for each.
    int filter_zero_points[] = {1, filter_zero_point};
    float filter_scales[] = {1, filter_scale};
    ...
    // Create per-tensor quantized int32_t bias tensor.
    tflite::SymmetricQuantize(kConvCommonZeroBias, bias_quantized16x16x28Inp, kNumFilters,
                              input_scale * output_scale);
    TfLiteTensor bias_tensor =
        tflite::testing::CreateTensor(bias_quantized16x16x28Inp, bias_dims);
    ...
    tflite::Quantize(kConvInput1x16x16x28Golden1x16x16x12, golden_quantized1x16x16x12, output_dims_count,
                     output_scale, output_zero_point);
...
TF_LITE_MICRO_TEST(Int8Input1x17x17x16Filter10x3x3x16Stride2x2ShouldMatchGolden) {
  using tflite::Int8ConvCommonInput;
  using tflite::Int8ConvCommonFilter;
  using tflite::kConvInput1x17x17x16Golden1x8x8x10Stride2x2;
  using tflite::kConvCommonZeroBias;
  using tflite::input_quantized1x17x17x16;
  using tflite::filter_quantized10x1x17x17x16;
  using tflite::output_quantized1x8x8x10;
  ...
  tflite::Quantize(kConvInput1x17x17x16Golden1x8x8x10Stride2x2, golden_quantized1x8x8x10, output_dims_count,
                   output_scale, output_zero_point);
...
TF_LITE_MICRO_TEST(Int8Input1x15x15x32Filter12x3x3x32Stride2x2gShouldMatchGolden) {
  using tflite::Int8ConvCommonInput;
  using tflite::Int8ConvCommonFilter;
  using tflite::kConvInput1x15x15x32Golden1x7x7x12Stride2x2;
  using tflite::kConvCommonZeroBias;
  using tflite::input_quantized1x15x15x32;
  using tflite::filter_quantized12x1x15x15x32;
  using tflite::output_quantized1x15x15x12;
  using tflite::golden_quantized1x15x15x12;
  using tflite::bias_quantized15x15x35Inp;
  constexpr int kNumFilters = 12;
  int input_shape[] = {4, 1, 15, 15, 32};  // [len,N,H,W,C]
  int filter_shape[] = {4, kNumFilters, 3, 3, 32};
  int bias_shape[] = {1, kNumFilters};
  int output_shape[] = {4, 1, 7, 7, kNumFilters};
  ...
  // Create per-tensor quantized int8_t input tensor.
  TfLiteTensor input_tensor = tflite::testing::CreateQuantizedTensor(
		  Int8ConvCommonInput, input_quantized1x15x15x32, input_dims, input_scale, input_zero_point);
  // Set zero point and scale arrays with a single element for each.
  int input_zero_points[] = {1, input_zero_point};
  float input_scales[] = {1, input_scale};
  TfLiteAffineQuantization input_quant = {
  ...
  // Create per-tensor quantized int8_t filter tensor.
  TfLiteTensor filter_tensor = tflite::testing::CreateQuantizedTensor(
		  Int8ConvCommonFilter, filter_quantized12x1x15x15x32, filter_dims, filter_scale,
      filter_zero_point);
  // Set zero point and scale arrays with a single element for each.
  int filter_zero_points[] = {1, filter_zero_point};
  float filter_scales[] = {1, filter_scale};
  ...
  tflite::Quantize(tflite::kConvInput1x15x15x32Golden1x7x7x12Stride2x2, golden_quantized1x15x15x12, output_dims_count,
                   output_scale,
				   output_zero_point);
...
TF_LITE_MICRO_TEST(Int8Input1x15x15x35Filter12x3x3x35Stride2x2ShouldMatchGolden) {
  using tflite::Int8ConvCommonInput;
  using tflite::Int8ConvCommonFilter;
  using tflite::kConvInput1x15x15x35Golden1x7x7x12Stride2x2;
  using tflite::kConvCommonZeroBias;
  using tflite::input_quantized1x15x15x35;
  using tflite::filter_quantized12x1x15x15x35;
  using tflite::output_quantized1x15x15x12;
  ...
  tflite::Quantize(tflite::kConvInput1x15x15x35Golden1x7x7x12Stride2x2, golden_quantized1x15x15x12, output_dims_count,
                   output_scale,
				   output_zero_point);
